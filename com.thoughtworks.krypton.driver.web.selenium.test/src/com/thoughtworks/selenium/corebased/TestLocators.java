...
//			verifyEquals("a2", selenium.getAttribute("link=this is the second element@class"));
			...
			boolean sawThrow46 = false;
			try {
				// originally verifyElementPresent|name=foobar|
						assertTrue(selenium.isElementPresent("name=foobar"));
			}
			catch (Throwable e) {
				sawThrow46 = true;
			}
			verifyFalse(sawThrow46);
			

			boolean sawThrow47 = false;
			try {
				// originally verifyElementPresent|id=foobar|
						assertTrue(selenium.isElementPresent("id=foobar"));
			}
			catch (Throwable e) {
				sawThrow47 = true;
			}
			verifyFalse(sawThrow47);
